...
@[obs_leaf]
theorem le_expect_one_of_le_mass {x : SPMF α} {p : α → ℝ≥0∞} {c : ℝ≥0∞} (hx : c ≤ x.mass)
...
@[obs_leaf]
theorem le_expect_of_le_mass {x : SPMF α} {p : α → ℝ≥0∞} {c d : ℝ≥0∞} (hx : c ≤ x.mass)
...
@[obs_leaf]
theorem le_expect_of_isPMF {x : SPMF α} {p : α → ℝ≥0∞} {d : ℝ≥0∞} (hx : IsPMF x)
    (hp : ∀ a, p a = d) : d ≤ expectObs.spec x p :=
  (one_mul d).ge.trans (le_expect_of_le_mass hx.ge hp)
...
@[obs_leaf]
theorem le_expect_iInf_of_le_mass {x : SPMF α} {p : α → ℝ≥0∞} {c : ℝ≥0∞} (hx : c ≤ x.mass) :
    c * ⨅ a, p a ≤ expectObs.spec x p :=
  (le_expect_of_le_mass hx fun _ => rfl).trans (expect_mono fun a => iInf_le p a)

@[obs_leaf, inherit_doc le_expect_iInf_of_le_mass]
theorem le_expect_iInf_of_isPMF {x : SPMF α} {p : α → ℝ≥0∞} (hx : IsPMF x) :
    ⨅ a, p a ≤ expectObs.spec x p :=
  (one_mul _).ge.trans (le_expect_iInf_of_le_mass hx.ge)
...
/-- Walk `goal`, `c ≤ (gen …).mass`, returning the arithmetic goal `c ≤ b` for the bound `b` the walk
computes, then whatever else the walk left. -/
partial def walkMass (extras : Array Term) (goal : MVarId) : TermElabM (List MVarId) :=
  goal.withContext do
  ...
  let g := rhs.getArg! 1
  if let some cases ← splitMatch? goal g then return ← cases.flatMapM (walkMass extras)
  -- `SPMF.expect g (fun _ => 1) = g.mass`
  let one ← mkAppOptM ``SPMF.expect_one #[none, g]
  let some (_, expectOne, _) := (← inferType one).eq? | throwError bad
  let expectGoal ← mkFreshExprMVar (mkApp2 ty.appFn!.appFn! (ty.getArg! 2) expectOne)
  goal.assign (← mkAppM ``le_of_le_of_eq #[expectGoal, one])
  arithBound extras ``SPMF.expectObs true g (expectOne.getArg! 2) (ty.getArg! 2) expectGoal.mvarId!
...
elab_rules : tactic
  | `(tactic| mass_bound $[$fs]?) => withMainContext do
...
list, as `le_expect_iInf_of_le_mass` does for a leaf. The unbounded-length ones (`le_expect_listOf`,
...
theorem le_expect_vectorOf {n : Nat} {g : SPMF α} {c d : ℝ≥0∞} {p : List α → ℝ≥0∞}
  ...
  le_expect_of_le_mass (pow_le_mass_vectorOf (hg.trans_eq (expect_one g))) hp
...
theorem le_expect_vectorOf_iInf {n : Nat} {g : SPMF α} {c : ℝ≥0∞} {p : List α → ℝ≥0∞}
  ...
  le_expect_iInf_of_le_mass (pow_le_mass_vectorOf (hg.trans_eq (expect_one g)))
...
theorem le_expect_listOfMaxLength {n : Nat} {g : SPMF α} {c d : ℝ≥0∞} {p : List α → ℝ≥0∞}
  ...
  le_expect_of_le_mass (pow_le_mass_listOfMaxLength (hg.trans_eq (expect_one g))) hp
