...
property and running it. A property is a generator that may reject its input, and so is polymorphic
in its monad, which means one property term is testable at every interpretation of `Gen`, and a
campaign is just that term run at a chosen one.
