...
def runOneIO (counters : IO.Ref (Nat × Nat)) (T : PropM FuzzGen Unit)
  ...
  -- `ofNatTruncate` is Lean 4.29's name for the saturating conversion later renamed `ofNatClamp`.
  noteDeficitImpl (UInt32.ofNatTruncate r.deficit) -- note any bytes deficit
