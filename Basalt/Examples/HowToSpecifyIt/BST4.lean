...
/-- Bug 4: `delete` fails to rebuild the tree above the key being deleted,
    returning only the remainder of the tree from that point on (an easy
    mistake for those used to imperative programming to make) -/
def delete (key : k) : BST k v → BST k v
  | Leaf => Leaf
  | Branch l k' _ r =>
    match compare key k' with
    | .lt => delete key l  -- Bug: doesn't rebuild with branch
    | .gt => delete key r  -- Bug: doesn't rebuild with branch
    | .eq => join l r
