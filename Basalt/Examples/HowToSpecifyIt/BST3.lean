...
/-- Bug 3: `insert` fails to update an existing key,
    leaving the tree unchanged instead -/
def insert (key : k) (val : v) : BST k v → BST k v
  ...
  | Branch l k' v' r =>
    ...
    | .eq => branch l k' v' r  -- Bug: keeps old value v' instead of new value val
