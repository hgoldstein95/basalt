...
namespace STLCMutants.STLC7

open Expr Ty

/-- Bug 7: subst_abs_no_shift - doesn't shift s in Abs case -/
...
def subst (n : Nat) (s : Expr) : Expr → Expr
  ...
  | Expr.Bool b => Expr.Bool b
  | Abs t e => Abs t (subst (n + 1) s e)  -- BUG: Should be (shift 1 s)
