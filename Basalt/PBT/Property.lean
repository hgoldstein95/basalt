...
/-- A `Bool` can be coerced to a property via `check`. -/
instance instCoeBoolPropM [Gen G] : CoeTail Bool (PropM G Unit) := ⟨check⟩

/-- A decidable `Prop` can also be coerced via `check` and `decide`. -/
...
/-- Run a `PropM` to get a `G TestOutcome`. -/
def runProp [Gen G] (p : PropM G Unit) : G TestOutcome := p.run

/-- A `Property` is a `G TestOutcome` that can be instantiated at any `G`. -/
