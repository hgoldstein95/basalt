...
theorem Tree.genUnsplay.terminates (s c k : Nat) (lo hi : Int) :
  ...
  induction k generalizing lo hi <;> rw [Tree.genUnsplay] <;> mass_bound <;>
...
theorem Tree.genUnsplay.cost_bounded (s c k : Nat) (lo hi : Int) :
  ...
  induction k generalizing lo hi <;> rw [Tree.genUnsplay] <;> cost_bound
...
theorem Tree.genSpecialUnsplay.terminates {n : Nat} {lo hi : Int} (h5 : 5 ≤ n) :
    IsAlmostSurelyTerminating (Tree.genSpecialUnsplay n lo hi h5) := by
...
theorem Tree.genSpecialMixed.terminates {n : Nat} {lo hi : Int} (h5 : 5 ≤ n) :
    IsAlmostSurelyTerminating (Tree.genSpecialMixed n lo hi h5) := by
  mass_fixpoint using SPMF.LfpIsOne.one
  simp [ENNReal.inv_two_add_inv_two]
