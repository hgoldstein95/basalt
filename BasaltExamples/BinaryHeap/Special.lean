...
/-! `genPath` and `genFill` recurse structurally, so their laws induct on the recursion's measure
and walk one step. -/

theorem genPath.terminates {lim : Nat} :
    ∀ (d i : Nat), IsAlmostSurelyTerminating (genPath lim i d : SPMF (List Nat)) := by
  intro d
  induction d with
  | zero => intro i; rw [genPath]; exact SPMF.IsPMF.of_one_le (by mass_bound; rfl)
  | succ d ih =>
    intro i
    rw [genPath]
    refine SPMF.IsPMF.of_one_le ?_
    mass_bound [ih]
    split <;> simp [ENNReal.inv_two_add_inv_two]

theorem genFill.terminates {n maxVal v : Nat} {ps : List Nat} :
    ∀ (fuel : Nat) (pre : List Nat),
      IsAlmostSurelyTerminating (genFill n maxVal v ps fuel pre : SPMF (List Nat)) := by
  intro fuel
  induction fuel with
  | zero => intro pre; rw [genFill]; exact SPMF.IsPMF.of_one_le (by mass_bound; rfl)
  | succ f ih =>
    intro pre
    rw [genFill]
    refine SPMF.IsPMF.of_one_le ?_
...
theorem genWorstCase.terminates {n maxVal : Nat} {hm : 1 ≤ maxVal} {hf : Feasible n} :
  ...
  rcases n with _ | _ | n <;> mass_fixpoint using SPMF.LfpIsOne.one
  all_goals simp

theorem genPath.cost_bounded {lim : Nat} :
    ∀ (d i : Nat), IsCostBounded (genPath lim i d) (fun _ => d) := by
  intro d
  induction d with
  | zero => intro i; rw [genPath]; cost_bound; omega
  | succ d ih =>
    intro i
    rw [genPath]
    cost_bound [ih]
...
theorem genFill.cost_bounded {n maxVal v : Nat} {ps : List Nat} :
    ∀ (fuel : Nat) (pre : List Nat),
      IsCostBounded (genFill n maxVal v ps fuel pre) (fun _ => fuel) := by
  intro fuel
  induction fuel with
  | zero => intro pre; rw [genFill]; cost_bound; omega
  | succ f ih =>
    intro pre
    rw [genFill]
    cost_bound [ih]
...
theorem genWorstCase.cost_bounded {n maxVal : Nat} {hm : 1 ≤ maxVal} {hf : Feasible n} :
  ...
  rcases n with _ | _ | n <;> cost_fixpoint
  all_goals simp only [Nat.add_assoc, Nat.reduceAdd] at *; omega
...
theorem genWorstCaseUpTo.cost_bounded {maxSize maxVal : Nat} {hm : 1 ≤ maxVal} :
  ...
  cost_fixpoint
  all_goals
    have hle := (mem_feasibleSizes.mp h_n).1
    have := Nat.log_mono_right (b := 2) hle
    omega
