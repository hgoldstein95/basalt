...
Basalt's formal definition of PBT generators allows for first-class coinductive fixed points. This
note gives the background on prior generator representations and explains how Basalt's relates to
them.
...
paths, since generators are expressed as Rocq functions and Rocq requires that all functions
terminate. The authors get around this by adding a fuel parameter, allowing generators to fail if
they run out of fuel, but this is messy and makes it difficult to talk about termination as a
first-class concept.
