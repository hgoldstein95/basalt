...
`BasaltFuzz/BuggyBST.lean`'s bugs are all *shallow* — one unlucky draw exposes them, so every backend
finds them in tens of runs and a comparison measures nothing. This module is the opposite case and
exists only to be measured: a bug behind a chain of guards, the shape a real bug takes behind a
parser, a magic value, or a sequence of state transitions, but synthetic so the depth is a knob.
`fuzz-run/compare-backends.sh` runs the comparison and `fuzz-run/README.md` records it.
-/
